class Challenge
  ...
  def quintuple_cave
    cave_2 = increment(cave)
    cave_3 = increment(cave_2)
    cave_4 = increment(cave_3)
    cave_5 = increment(cave_4)
    @cave = (cave + cave_2 + cave_3 + cave_4 + cave_5).transpose
  end

  def increment(cave)
    cave.map { |row| row.map { |digit| digit == 9 ? 1 : digit + 1 }}
  end
